...
public class Overflow {
    ...
    private static List<Float> power_fp(float n, int m) {
        ...
        for (int j = 1; j <= m; j++) {
            ns = ns * n;
            assert !Float.isInfinite(ns) : "Floating-point overflow"; // Lab - 3 edits
            resultList.add(ns);
        }
        return resultList;
    }
    ...
    private static List<Float> geom_fp(float n, int m) {
        ...
        for (int j = 0; j <= m; j++) {
            ...
            assert ns_inv != 0.0f : "Floating point underflow"; // Lab - 3 edits
    ...
    public static void main(String[] args) {
        ...
        int t = getPositiveInteger(
                "task (1=floating-point n^m; 2=integer n^m; 3=floating-point 1+1/n+1/n^2+...+1/n^m)");

        switch (t) {
            case 1:
                System.out.println(power_fp(n, m));
                break;
            case 2:
                System.out.println(power(n, m));
                break;
            case 3:
                System.out.println(geom_fp(n, m));
        }
    }
}
